...
if (exists("train", envir = .GlobalEnv)) {
  train_sf <- get("train", envir = .GlobalEnv)
  if (!inherits(train_sf, "sf")) {
    train_sf <- st_as_sf(train_sf, coords = c("lon", "lat"), crs = 4326, remove = FALSE)
  }
} else {
  train_sf <- readRDS(here(paths$processed, "train_model.rds")) |>
...
pesos_6km     <- readRDS(here(paths$cv, "pesos_dist_6km.rds"))
pesos_uniform <- readRDS(here(paths$cv, "pesos_uniform.rds"))
...
# LOG
# =============================================================
# Nota: mae_directo está en escala COP (price), no log_price.
# No es directamente comparable con MAE de modelos tidymodels.

log_modelo(nombre = "Super_v1_uniform",     mae_directo = mae_norte_v1_uniform)
log_modelo(nombre = "Super_v1_6km",         mae_directo = mae_norte_v1_6km)
log_modelo(nombre = "Super_v2_conservador", mae_directo = mae_norte_cons)
log_modelo(nombre = "Super_v2_agresivo",    mae_directo = mae_norte_agr)

message("06_super_learning.R — listo")
